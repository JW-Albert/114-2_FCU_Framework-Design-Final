...
sequenceDiagram
    ...
    %% ── 2. 管理員核准 ──
    管理員->>BC: POST /api/borrowings/{id}/approve
    BC->>BS: approveRequest(admin, id, note)
    BS->>Repo: findById(id)
    Repo-->>BS: BorrowingRequest{PENDING}
    note over BS: State Pattern：委派給 PendingState
    ...
    %% ── 3. 出車 ──
    管理員->>BC: POST /api/borrowings/{id}/start
    BC->>BS: startUse(id)
    BS->>BS: vehicle.markInUse()
    note over BS: State Pattern：委派給 ApprovedState
...
classDiagram
    ...
    note for UserRepositoryAdapter "Adapter Pattern：\n將 JPA 介面適配為\nDomain 所需的\nIUserRepository 介面"
    note for InMemoryUserRepository "測試用實作：\n不依賴資料庫，\n注入 Service 進行單元測試（DIP）"

    IUserRepository <|.. UserRepositoryAdapter : Adapter Pattern
    IUserRepository <|.. InMemoryUserRepository : 單元測試用
    UserRepositoryAdapter --> JpaUserRepo : delegates
    JpaUserRepo --|> JpaRepository : extends
```
